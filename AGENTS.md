...
- **Guardrails** (`guardrails:global` key, read via `mcp/guardrails.ts`, enforced inline in `place_limit_order` / `place_market_order` / `place_maker_reward_order` / `place_optimized_reward_order` before any SDK call): **default is readOnly — all placement blocked — until the owner explicitly calls `update_strategy({ tokenId: "guardrails:global", readOnly: false, ... })`.** This is a safety default for a server that ships to third-party operators/agents with a funded wallet, not an opt-in nicety. Once configured, checks also cover maxOrderSizeUsd, maxPriceDeviationFromMid, allowedTokenIds, maxOpenOrdersTotal. Blocks return `{blocked, reason, agentDirective}` with `isError: true`. See README.md "Safety" section and `src/mcp/guardrails.ts`.
